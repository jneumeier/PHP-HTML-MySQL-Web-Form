 <?php
	...
	// ------------------------------------------------------------------------------------
	// Name: getTeams
	// Abstract: Retrieves an array with all of the teams in the database.
	// ------------------------------------------------------------------------------------
	function getTeams() {
		// create database credentials
		$hostname = "xxxxxxxxxxxxxxxxx";
		...
		//if query fails stop further execution and show mysql error
		$query = $conn->query("SELECT strTeamName FROM TTeams ORDER BY strTeamName")
				or die(mysql_error());
		...
		if($query->num_rows <= 0) { $query = -1; }
			...
			while( $row = $query->fetch_assoc() ) {
				...
				// get team name from query's current row
				$strTeamName = $row["strTeamName"];
					
				if($intIndex == 0) {
					
					# put initial variable into an array
					$arrTeamNames = array($strTeamName);
					
				} else {		
				
					// append to array
					array_push($arrTeamNames, $strTeamName);
				}
